...
func _apply_materials() -> void:
	var mesh_instances :=  _get_all_mesh_instances(get_parent())
...
func _get_all_mesh_instances(node: Node, children_acc: Array[MeshInstance3D] = []) -> Array[MeshInstance3D]:
	if node is MeshInstance3D:
		children_acc.push_back(node)
		
	for child in node.get_children():
		children_acc = _get_all_mesh_instances(child, children_acc)
